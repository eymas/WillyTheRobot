...
while not rospy.is_shutdown(): 
    ...
    if(re.search(r"\d+\|\d+\|\d+", topicMessage)):
		...
		PostOnTopic("/sideRight",float(topicMessage.split("|")[3]))
		PostOnTopic("/backRight",float(topicMessage.split("|")[4]))
		PostOnTopic("/backMiddle",float(topicMessage.split("|")[5]))
		PostOnTopic("/backleft",float(topicMessage.split("|")[6]))
		PostOnTopic("/sideLeft",float(topicMessage.split("|")[7]))

...
ROS uses 0.01 as a default value, since the covariance is not noted in any datasheets, and the calculations to find them are almost a study onto themselves.

The basic process is shown in figure \ref{fig::piDataReception}.

\begin{figure}[H]
\includegraphics[width=12cm]{temp.jpg} \TODO{change image}
\caption{The process by which incoming data is handled on the Pi}
\label{fig::piDataReception}
\end{figure}

Since the data recorded by the arduino is stored in a float format, which is not transmittable by serial communications.
...
One Tesla is $1.0 X 10^7$ milliGauss, so in order to convert mG into T, the opposite needs to be done, $1.0 X 10^-7$.
